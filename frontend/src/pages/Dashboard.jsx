...
export default function Dashboard() {
  ...
  // ─── Widgets según rol y datos reales ───────────────────────────────────────
  const buildWidgets = () => {
    if (!stats) return [];
    if (user.role === "admin") {
      return [
  ...
  return (
    <div className="db-root">
      ...
      <aside className="db-sidebar">
        ...
        {/* Brand — solo texto + punto de color */}
        <div className="db-brand">
          <span className="db-brand-dot" />
          <span className="db-brand-name">EduControl</span>
        </div>

        {/* Navegación */}
        <nav className="db-nav">
          {Object.entries(grouped).map(([section, items]) => (
            <div key={section} className="db-nav-group">
              {section !== "__top__" && (
                <p className="db-nav-section">{section}</p>
              )}
              {items.map((item) => (
                <button
                  key={item.label}
        ...
        {/* Footer del sidebar: configuración + usuario + logout */}
        <div className="db-sidebar-footer">
          ...
          <div className="db-user-row">
            ...
            <div className="db-user-text">
              ...
              <span className={`db-role-tag ${roleColor[user.role]}`}>
                {roleLabel[user.role]}
              </span>
            </div>
            <button className="db-logout-icon" onClick={handleLogout} title="Cerrar sesión">
              <Icon name="logout" />
            </button>
          </div>
        </div>
        ...
        <div className="db-lower">
          ...
          <div className="db-card db-quick-card">
            ...
            <div className="db-quick-grid">
              {navItems.filter(i => i.label !== "Inicio").slice(0, 4).map((item) => (
                <button
                  key={item.label}
